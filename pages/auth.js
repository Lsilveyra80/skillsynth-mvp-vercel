  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          redirectTo: `${window.location.origin}/dashboard`, // cambia la ruta si usás otra
        },
      });
      if (error) throw error;
      // Supabase redirige solo
    } catch (err) {
      console.error(err);
    ...
    try {
      const { data, error } = await supabase.auth.signUp({
        ...
        options: {
          ...
          emailRedirectTo: `${window.location.origin}/dashboard`, // adónde vuelve después de confirmar
        },
      });
      ...
      if (data.user && !data.session) {
        // Si tenés activada la confirmación de email, no crea sesión hasta que confirme
        setMessage(
  ...
  const handleEmailLogin = async (e) => {
    ...
    try {
      ...
      if (data.session) {
        ...
        // Redirigís manualmente si querés:
        window.location.href = "/dashboard"; // cambia la ruta si usás otra
      }
    } catch (err) {
      console.error(err);
